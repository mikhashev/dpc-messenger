<script lang="ts">
  ...
  import { connectionStatus, nodeStatus, coreMessages, p2pMessages, sendCommand, connectToCoreService } from "$lib/coreService";
  
  // --- THE CORE FIX: Connect only onMount ---
  onMount(() => {
    // Attempt to connect once when the application loads.
    connectToCoreService();
  });

  function handleReconnect() {
    // The retry button now explicitly calls the connect function.
    connectToCoreService();
  }
