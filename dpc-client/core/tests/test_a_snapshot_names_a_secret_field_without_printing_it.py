...
@pytest.fixture()
def _snapshot(_chromium, tmp_path):
...
def test_a_select_marked_secret_by_autocomplete_is_withheld(_snapshot):
    ...
    assert re.fullmatch(
        r'\s*- combobox "Expiry month" = \[withheld\] \[@e\d+\]', line
    ), line
    ref = re.search(r"@e\d+", line).group(0)
    assert refs[ref]["role"] == "combobox"
    assert refs[ref]["name"] == "Expiry month"
    # The option is a node of its own and is named by its text, not by its
    # value attribute, so the chosen month stays readable as "July" while
    # "exp-month-07" does not leave the page.
    assert "July" in tree
...
_CLOSED_ROWS: tuple[_Row, ...] = (
    ...
    _Row("select-name-cc-number",
         ...
         '<option value="closed-select-13" selected>Chosen</option></select>',
