...
from .conversation_monitor import history_status_for as conversation_monitor_history_status_for
                ...
                for browser in live_browsers:
                    try:
                        ...
                        logger.warning(
                            ...
                            "force-killing the browser subprocess so it does "
                            "not orphan"
                        )
                        try:
                            browser._force_kill_process()
                        except Exception:
                            pass
                        try:
    ...
    async def on_peer_list_change(self):
        ...
        for peer_id in self.p2p_manager.peers:
            ...
            for group in shared_groups:
                ...
                # Monitors are created lazily, so the usual case on connect is
                # that none is loaded and the builder reads the file. The digest
                # covers this node's own window; the peer answers over the
                # pair's window once it has seen our boundary.
                status_payload = conversation_monitor_history_status_for(
                    group.group_id, self.conversation_monitors.get(group.group_id)
                )
                local_hash = status_payload["history_hash"]
                local_count = status_payload["message_count"]
