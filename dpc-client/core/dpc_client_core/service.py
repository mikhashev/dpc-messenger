...
class CoreService:
    ...
    async def web_auth_popup_complete(
        self,
        request_id: str,
        ...
        """T9 receiver side: frontend reports back after the user closed
        the Tauri popup that was opened for anti-bot challenge fallback.

        Looks up the matching pending request (registered by
        `_request_popup_fallback` in `browser.py`) and resolves its
        future with either the popup HTML or an `AuthRequiredError`.

        Q2 URL-safety check (Ark softer version, S142):
          - missing pending request → orphaned popup (likely backend
            restart per Q3 ephemeral design) → log + ack OK, the
            frontend should already show "request expired"
          - frontend reported `error` → set_exception, agent surfaces it
          - `current_url` eTLD+1 mismatches `expected_etld1` → reject
            with AuthRequiredError("navigated to different site")
          - eTLD+1 match, path mismatch → WARN log + accept the HTML;
            user might have made useful intra-site navigation
          - exact URL match → silently accept the HTML
        """
        from urllib.parse import urlparse

        from .dpc_agent.tools import browser as _browser
        from . import web_auth as _wa

        pending_map = _browser.get_pending_popup_requests()
        entry = pending_map.get(request_id)
        if entry is None:
            # Orphaned popup — no matching pending request. Ack so the
            # frontend can dismiss its loading state without crashing.
            logger.warning(
                "web_auth_popup_complete: unknown request_id=%s "
                "(likely backend restart between request and reply)",
                request_id,
            )
        ...
        if error:
            ...
            return {"status": "error_propagated", "request_id": request_id}

        if not content_html:
            entry.future.set_exception(
                _browser.AuthRequiredError(
                    "Popup reported no content_html — closed without rendering?"
                )
            )
            return {"status": "no_content", "request_id": request_id}

        current_url = current_url or ""
        actual_etld1 = _wa.resolve_etld1(current_url) if current_url else ""

        if not actual_etld1 or actual_etld1 != entry.expected_etld1:
            entry.future.set_exception(
                _browser.AuthRequiredError(
                    f"User navigated to different site "
                    f"(expected={entry.expected_etld1}, got={actual_etld1 or '<unparseable>'})"
                )
            )
            return {"status": "domain_mismatch", "request_id": request_id}

        # Same eTLD+1 — accept. Log a warning when the path moved so an
        # operator can later correlate "agent got unexpected content"
        # with "user wandered off /my/orders inside the popup".
        if urlparse(current_url).path != urlparse(entry.expected_url).path:
            logger.warning(
                "web_auth_popup_complete: user navigated from %s to %s "
                "within domain — accepting HTML anyway (Q2 softer)",
                entry.expected_url, current_url,
            )
    ...
    async def _handle_transcription_request(self, peer_id: str, request_id: str, audio_base64: str, mime_type: str, model: str = None, provider: str = None, language: str = "auto", task: str = "transcribe"):
        """Delegated to P2PCoordinator."""
        await self.p2p_coordinator.handle_transcription_request(peer_id, request_id, audio_base64, mime_type, model, provider, language, task)

    async def _handle_get_providers_request(self, peer_id: str):
        """Delegated to P2PCoordinator."""
        await self.p2p_coordinator.handle_get_providers_request(peer_id)

    async def _handle_providers_response(self, peer_id: str, providers: list):
        """Delegated to P2PCoordinator."""
        await self.p2p_coordinator.handle_providers_response(peer_id, providers)
