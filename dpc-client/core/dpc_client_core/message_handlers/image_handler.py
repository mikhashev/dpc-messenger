"""Handler for FILE_OFFER command with image metadata - P2P image transfers."""
...
class ImageOfferHandler(MessageHandler):
    """
    Handles FILE_OFFER messages for image transfers (extends regular file transfer).

    This handler processes FILE_OFFER commands and provides special handling for images:
    - Detects images by mime_type (image/*) and presence of image_metadata
    - Broadcasts image_offer_received event with thumbnail for images
    - Falls back to regular file transfer handling for non-images
    """
    ...
    async def handle(self, sender_node_id: str, payload: Dict[str, Any]) -> Optional[Any]:
        """
        Handle FILE_OFFER message (both images and regular files).
        ...
        For regular files (no image_metadata):
        1. Check firewall permission
        ...
        Args:
            ...
            payload: Contains:
                ...
                - image_metadata: dict (optional, for images)
        ...
        for existing_transfer_id, existing_transfer in file_transfer_manager.active_transfers.items():
            if (existing_transfer.node_id == sender_node_id and
                ...
                self.logger.warning(
                    f"Ignoring duplicate {'IMAGE' if is_image else 'FILE'}_OFFER from {sender_node_id} "
        ...
        if not allowed:
            self.logger.warning(f"{'Image' if is_image else 'File'} transfer denied by firewall from {sender_node_id}")
        ...
        self.logger.info(f"{'Image' if is_image else 'File'} transfer pending user acceptance: {transfer_id}")
        return None
