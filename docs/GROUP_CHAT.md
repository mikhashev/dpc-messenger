...
- **The reason given above no longer holds once the live-history boundary exists.** A member that took no part in the reset still keeps its history, but its records now arrive older than the boundary of the nodes that reset and go to their archives, not back into their live histories. The unanimity rule itself stays for now; whether it should is open on `ENDING-A-SESSION-NEEDS-EVERY-MEMBER-ONLINE-SO-THE-WAY-THROUGH-IS-REMOVING-THE-OFFLINE-ONE`.
