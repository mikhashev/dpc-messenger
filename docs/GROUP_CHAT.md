...
- **A refusal is never held.** `reject` reaches consensus whatever the voter holds — declining to sign needs no evidence. `approve` and `request_changes` are judgements about the text and stay held until the window is complete.
